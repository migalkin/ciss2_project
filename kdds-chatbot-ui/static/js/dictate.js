	...
	var Dictate = function(cfg) {
		...
		this.init = function() {
			...
			try {
				...
				console.log('Error while creating audioContext')
		...
		this.startListening = function() {
			...
			if (ws) {
				console.log('Websocket already exists, doing nothing')
			}else{
				try {
					ws = createWebSocket();
				} catch (e) {
		...
		this.submitReference = function submitReference(text, successCallback, errorCallback) {
			...
			sendReferenceToReferenceHandler(config, headers, text, successCallback, errorCallback)			
		}
		...
		this.resetHotword = function(){
			var resetHotword = { "hotword" : {"detected" : false}}
			socketSend(JSON.stringify(resetHotword));
		}
		...
		/** Setting the hotword for the online speech recognition
		 * hotword String containing one or multiple hotwords
		 * per_segment boolean True if the hotword detection should be done per segment
		 */
		this.setHotword = function setHotword(hotword, per_segment) {
			// check if we have one or multiple hotwords
			var hotwords = hotword.split(",");
			
			// if there is at least a single hotword
			if(hotwords.length > 0){	
				hotwords = hotwords.map(s => s.trim())
				var item = { hotword: { hotword: hotwords, per_segment: per_segment }};
	            socketSend(JSON.stringify(item));
			}else{
				console.log('No hotword was set, received following input - hotword: ' + hotword)
			}
